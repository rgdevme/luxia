...
export async function runSkill(opts: SkillOptions): Promise<void> {
  ...
  switch (opts.sub) {
    case "add": {
      ...
      if (!opts.noInstall) await dispatchSkillAdded(item, agents, ctx);
      break;
    }
    case "remove": {
      ...
      if (!opts.noInstall) await dispatchSkillRemoved(name, agents, ctx);
      break;
    }
    case "update": {
      const name = opts.args[0];
      ...
      if (!opts.noInstall) await dispatchSkillUpdated(item, agents, ctx);
      break;
    }
    case "list":
    case undefined: {
